...
def cos_correction(signal):
    ...
    signal_10 = 0.4*length # We calculate the length of a tenth of the signal.
...
def _filtering(signal,rate):
    ...
    low = filter_signal(signal, 15, rate, order=4, filtertype='lowpass') # We apply a low-pass filter.
